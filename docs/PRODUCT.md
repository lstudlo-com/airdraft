...
|---|---|
| 1 | All page content cards share an 18-point continuous corner radius. Airdraft branding throughout the app, CLI and developer tools; the sidebar brand is a compact 51-by-25-point neumorphic capsule in a 30-point row, with an outline-only raised rim, an unfilled interior that exposes the sidebar directly, five raised waveform bars and a separated caret, and the accessible name Airdraft; compact Codex-inspired sidebar with monochrome symbols, quiet selection and a subtle half-point divider, a toggle beside the traffic lights, and a microphone picker above the bottom word count; content begins without an empty title row; native cards, key caps and theme/HUD previews with an edge-to-edge Auto theme split; dark recording pill at bottom centre with Classic / Mini / None. Configuration and Models share equal 16-point card insets, 32-point minimum heading rows with a 4-point leading text inset, 12-point heading gaps and 28-point section gaps; settings rows add no extra vertical inset. The menu bar keeps status labels compact and shows Unload models only when a managed model is loaded; full model details remain in the main window. Control + Option is the default hold-to-dictate shortcut; saved shortcuts are preserved. Existing settings and data survive the rename. Auto, Light and Dark update both the main app and isolated interactive preview windows immediately. Native Close and Minimize buttons have 16-point top and leading insets, with the sidebar toggle aligned to their centers in a 46-point titlebar. Onboarding still missing. |
...
| 4 | Done: `RefinementProfile` + `ProfileStore`; Profiles pairs a compact list with an open editor. The base system prompt is visible above the profile list and editor, with a warning before editing. The list provides the only profile title; the editor starts with Refine transcript, Profile instructions and optional Task. One menu holds profile actions and icon selection. The sidebar uses a document-and-pencil symbol. |
...
| 10 | Implemented: macOS frosted sidebar material beneath a darker neutral layer that stays 70% opaque through the upper half and fades to fully opaque at the bottom, with a fully opaque Reduce Transparency fallback, a bottom-left settings button, a centered account/subscription overlay with labeled sample data, and a larger microphone chooser with a live input meter. The level preview retains no audio. |
...
- The sidebar uses native blur beneath a darker neutral layer that stays 70%
  opaque through the upper half, then fades to full opacity at the bottom.
  Reduce Transparency makes the entire background fully opaque.
