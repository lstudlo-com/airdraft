...
//
// Keys come from AIRDRAFT_ASR_KEY and AIRDRAFT_LLM_KEY. --key / --llm-key still work,
// but command-line arguments are visible to other processes and in shell history.
...
func makeTranscriber(_ args: Args) -> any Transcriber {
    ...
    if let base = args["url"], args["whisper"] == nil {
        ...
        if url.host == "api.openai.com", model == "gpt-transcribe" { return OpenAITranscriber(apiKey: asrKey) }
        return OpenAICompatibleTranscriber(baseURL: url, model: model, apiKey: asrKey)
    }
...
func makeRefiner(_ args: Args) -> any Refiner {
    ...
    return OpenAICompatibleRefiner(
        ...
        apiKey: args["llm-key"] ?? ProcessInfo.processInfo.environment["AIRDRAFT_LLM_KEY"],
...
let asrKey = args["key"] ?? ProcessInfo.processInfo.environment["AIRDRAFT_ASR_KEY"]
